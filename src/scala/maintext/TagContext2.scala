...
import Tagcontext.{TagKeyWord, TagsAd, TagsApp, Tagsarea, Tagschannel, Tagsequitment}
  ...
  def main(args: Array[String]): Unit = {
    ...
    df.filter(TagUtils.OneUserId)
      ...
      row.map(row=>{
        ...
        val dvList = Tagsequitment.makeTags(row)
        val loactionList = Tagsarea.makeTags(row)
